...
use crate::network::download::Download;
use crate::network::upload::Upload;

pub(self) const ALPN_FILE_SERVER:&[&[u8]] = &[b"file-server"];
...
impl Server {
    ...
    pub(crate) async fn run(&mut self) -> anyhow::Result<()> {
        ...
        let mut rust_tls_config = rustls::ServerConfig::builder()
            .with_safe_defaults()
            .with_no_client_auth()
            .with_single_cert(vec![cert],key)?;
        rust_tls_config.alpn_protocols = ALPN_FILE_SERVER.iter().map(|&x| x.into()).collect();
        let ctrl_listener = TcpListener::bind(
            format!("0.0.0:{}",ctrl_port)
...
pub(self) struct Request {
    ...
    cmd_map:Arc<DashMap<String,Sender<Cmd>>>,
...
impl Request {
    ...
    pub(self) async fn handle(&mut self) -> anyhow::Result<()> {
        ...
        loop {
            ...
            let content = match self.stream
                .read_exact(&mut buffer[0 ..req_len as usize]).await{
                Ok(n) => {
            ...
            match op_code{
                ...
                4 => {
                    ...
                    match self.cmd_map.get(session_id.as_str()) {
                        ...
                        Some(cmd_tx) => {
                            ...
                            self.stream
                                .write_all(format!("ok {} {}\n",metadata.filename,metadata.size).as_bytes()).await?;
                            break;
                        }
                    }
                }
                5 => {
                    ...
                    match self.cmd_map.get(session_id.as_str()){
                        ...
                        Some(cmd_tx) => {
                            cmd_tx.send(Cmd::DownloadDirectly(metadata.filepath,metadata.size as usize)).await?;
                            self.stream
...
pub(self) struct UnsafePlaceholder<T>{
        ...
        loop {
            match self.cmd_rx.recv().await{
                ...
                Some(cmd) => match cmd{
                    ...
                    Cmd::DownloadDirectly(filename, usize) => {
                        self.download_directly(filename.as_str(),usize).await?
                    }
                }
            }
        }
